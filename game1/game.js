...
function createPieces() {
    ...
    piece.style.cssText = 'position:absolute;width:32px;height:32px;background:rgba(255,255,255,0.85);border:2px solid #ff4444;border-radius:50%;display:flex;align-items:center;justify-content:center;z-index:100;box-shadow:0 2px 8px rgba(0,0,0,0.3);font-size:18px;';
    const img = gameState.pieceImage;
    piece.innerHTML = img ? `<img src="${img}" style="width:26px;height:26px;border-radius:50%;object-fit:cover;">` : gameState.pieceIcon;
...
function finishMove() {
    gameState.isRolling = false;
    document.getElementById('rollBtn').disabled = false;
    updateMessage('继续掷骰子前进！');
}

// 更新玩家显示
function updatePlayerDisplay() {
    document.querySelector('.player1').classList.add('active');
    const status = document.querySelector('.player1 .player-status');
    if (status) {
        const pos = gameState.players[1].position;
...
function saveCellEffect() {
    ...
    localStorage.setItem('g1_cellEffects', JSON.stringify(gameState.cellEffects));
    localStorage.setItem('g1_cellPopups', JSON.stringify(gameState.cellPopups));
...
function restartGame() {
    ...
    const savedIcon = gameState.pieceIcon;
    const savedImg = gameState.pieceImage;
...
function clearAllData() {
    localStorage.removeItem('g1_cellEffects');
    localStorage.removeItem('g1_cellPopups');
    localStorage.removeItem('g1_pieceIcon');
    localStorage.removeItem('g1_pieceImage');
    gameState.cellEffects = {};
    gameState.cellPopups = {};
    gameState.pieceIcon = '👠';
    gameState.pieceImage = null;
    initBoard();
...
// 棋子自定义
function setupPieceCustomization() {
    const select = document.getElementById('pieceIcon');
    ...
    select.addEventListener('change', function(e) {
        if (e.target.value === 'image') {
            ...
            gameState.pieceIcon = e.target.value;
            gameState.pieceImage = null;
            updatePieceIcon();
            localStorage.setItem('g1_pieceIcon', e.target.value);
            localStorage.removeItem('g1_pieceImage');
        }
    });
    ...
    fileInput.addEventListener('change', function(e) {
        ...
        if (file) {
            const reader = new FileReader();
            reader.onload = function(event) {
                gameState.pieceImage = event.target.result;
                updatePieceIcon('image');
                localStorage.setItem('g1_pieceImage', event.target.result);
            };
            reader.readAsDataURL(file);
        }
    });
...
function updatePieceIcon(type) {
    const piece = document.getElementById('piece1');
    if (!piece) return;
    const img = gameState.pieceImage;
    if (type === 'image' && img) {
        piece.innerHTML = `<img src="${img}" style="width:26px;height:26px;border-radius:50%;object-fit:cover;">`;
    } else {
        piece.innerHTML = gameState.pieceIcon;
    }
}

setupPieceCustomization();

// 初始化游戏
initBoard();

// 从localStorage加载保存的数据
const savedEffects = localStorage.getItem('g1_cellEffects');
const savedPopups = localStorage.getItem('g1_cellPopups');
...
const savedIcon = localStorage.getItem('g1_pieceIcon');
const savedImg = localStorage.getItem('g1_pieceImage');
if (savedImg) { gameState.pieceImage = savedImg; updatePieceIcon('image'); }
else if (savedIcon) { gameState.pieceIcon = savedIcon; document.getElementById('pieceIcon').value = savedIcon; updatePieceIcon(); }
