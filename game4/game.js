...
for (let p = 1; p <= NUM_PLAYERS; p++) {
    gameState.players[p] = { position: -1, skipTurn: false };
    gameState.pieceIcons[p] = PLAYER_ICONS[p-1];
    gameState.pieceImages[p] = null;
}
...
function initBoard() {
    ...
    // 创建棋子
    createPieces();
    for (let p = 1; p <= NUM_PLAYERS; p++) placePiece(p, -1);
}
...
function createPieces() {
    ...
    for (let p = 1; p <= NUM_PLAYERS; p++) {
        ...
        piece.style.cssText = `position:absolute;width:28px;height:28px;background:rgba(255,255,255,0.85);border:2px solid ${borders[p-1]};border-radius:50%;display:flex;align-items:center;justify-content:center;z-index:100;box-shadow:0 2px 8px rgba(0,0,0,0.3);font-size:14px;`;
        const img = gameState.pieceImages[p];
        piece.innerHTML = img ? `<img src="${img}" style="width:22px;height:22px;border-radius:50%;object-fit:cover;">` : gameState.pieceIcons[p];
...
function movePiece(player, steps) {
    ...
    function jumpAnimation(currentTime) {
        ...
        if (progress < 1) {
            ...
            // 🎯 碰撞检测：检查所有其他玩家
            for (let op = 1; op <= NUM_PLAYERS; op++) {
                if (op === player) continue;
                const od = gameState.players[op];
                if (od.position === targetPos && targetPos > 0 && targetPos < pathLength - 1) {
                    od.position = -1;
                    placePiece(op, -1);
                    updateMessage(`💥 玩家${player}撞飞了玩家${op}！`);
                    setTimeout(() => {
                        const se = document.querySelector(`.player${op} .player-status`);
                        if (se) se.textContent = '被撞回起点';
                    }, 100);
                }
            }
...
function switchPlayer() {
    ...
    gameState.currentPlayer = (gameState.currentPlayer % NUM_PLAYERS) + 1;
    updatePlayerDisplay();
    document.getElementById('rollBtn').disabled = false;
    updateMessage(`轮到 ${PLAYER_COLORS[gameState.currentPlayer-1]}玩家${gameState.currentPlayer} 掷骰子`);
}
...
function saveCellEffect() {
    ...
    localStorage.setItem('g4_cellEffects', JSON.stringify(gameState.cellEffects));
    localStorage.setItem('g4_cellPopups', JSON.stringify(gameState.cellPopups));
...
function restartGame() {
    ...
    const savedImages = {};
    for (let p = 1; p <= NUM_PLAYERS; p++) {
        savedIcons[p] = gameState.pieceIcons[p];
        savedImages[p] = gameState.pieceImages[p];
    }

    gameState = {
        currentPlayer: 1, diceValue: 0, isRolling: false, gameOver: false,
        players: {},
        cellEffects: savedEffects, cellPopups: savedPopups,
        pieceIcons: savedIcons, pieceImages: savedImages
    };
    for (let p = 1; p <= NUM_PLAYERS; p++) {
        if (!gameState.players[p]) gameState.players[p] = { position: -1, skipTurn: false };
    }
...
function clearAllData() {
    localStorage.removeItem('g4_cellEffects');
    localStorage.removeItem('g4_cellPopups');
    for (let p = 1; p <= NUM_PLAYERS; p++) {
        localStorage.removeItem('g4_pieceIcon' + p);
        localStorage.removeItem('g4_pieceImage' + p);
    }
...
function setupPieceCustomization(player) {
    ...
    select.addEventListener('change', function(e) {
        if (e.target.value === `image${player}`) {
            ...
            gameState.pieceIcons[player] = e.target.value;
            gameState.pieceImages[player] = null;
            updatePieceIcon(player);
            localStorage.setItem('g4_pieceIcon' + player, e.target.value);
            localStorage.removeItem('g4_pieceImage' + player);
        }
    });
    ...
    fileInput.addEventListener('change', function(e) {
        ...
        if (file) {
            const reader = new FileReader();
            reader.onload = function(event) {
                gameState.pieceImages[player] = event.target.result;
                updatePieceIcon(player, 'image');
                localStorage.setItem('g4_pieceImage' + player, event.target.result);
            };
            reader.readAsDataURL(file);
        }
    });
...
function updatePieceIcon(player, type) {
    const piece = document.getElementById(`piece${player}`);
    if (!piece) return;
    const img = gameState.pieceImages[player];
    if (type === 'image' && img) {
        piece.innerHTML = `<img src="${img}" style="width:22px;height:22px;border-radius:50%;object-fit:cover;">`;
    } else {
        piece.innerHTML = gameState.pieceIcons[player];
    }
}

for (let p = 1; p <= NUM_PLAYERS; p++) setupPieceCustomization(p);

initBoard();

const savedEffects = localStorage.getItem('g4_cellEffects');
const savedPopups = localStorage.getItem('g4_cellPopups');
if (savedEffects) { gameState.cellEffects = JSON.parse(savedEffects); for (let i = 0; i < gamePath.length; i++) updateCellDisplay(i); }
if (savedPopups) gameState.cellPopups = JSON.parse(savedPopups);

for (let p = 1; p <= NUM_PLAYERS; p++) {
    const si = localStorage.getItem('g4_pieceIcon' + p);
    const sim = localStorage.getItem('g4_pieceImage' + p);
    if (sim) { gameState.pieceImages[p] = sim; updatePieceIcon(p, 'image'); }
    else if (si) { gameState.pieceIcons[p] = si; document.getElementById('pieceIcon' + p).value = si; updatePieceIcon(p); }
}
